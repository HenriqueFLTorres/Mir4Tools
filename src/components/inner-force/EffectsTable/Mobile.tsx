...
export default function MobileEffectsTable({
  ...
  return effectsObject.length > 0 ? (
    <ul className="custom-scroll relative flex w-full grid-cols-2 flex-col gap-1 overflow-auto rounded-md bg-primary-600 p-1 sm:grid md:w-[43rem] xl:hidden">
      {effectsObject.map(([name, value]) => {
        ...
        return (
          ...
          >
            <Icon className="hidden h-6 w-6 shrink-0 sm:flex" />{' '}
